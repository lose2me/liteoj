...
// 共用的提交列表组件。所有调用点（/me 的"我的提交"、/admin/submissions、
// 题单提交 tab）走同一份 columns，区别只有三处：
...
const props = withDefaults(
  defineProps<{
    ...
    hideFiltersUser?: boolean;
    pageSize?: number;
  }>(),
  {
    showFilters: false,
    detailFromPs: false,
    hideFiltersUser: false,
    pageSize: 16,
  },
);
...
onMounted(async () => {
  ...
  off = onEvent((ev) => {
    if (
      ev.type === "submission:new" ||
      ev.type === "submission:done" ||
      ev.type === "ai:task:done"
    ) {
      load();
    }
...
const canView = (r: any) =>
  (user.isAdmin || r.user_id === user.user?.id) && !r.ai_pending;
...
const columns = computed(() => {
  ...
  cols.push(
      ...
      render: (r: any) =>
        h(
          "span",
          {
            ...
            onClick: (e: Event) => {
              e.stopPropagation();
              router.push(`/problems/${r.problem_id}`);
            },
          },
          `#${r.problem_id}`,
        ),
    },
    {
      ...
      render: (r: any) => {
        const children: any[] = [
          h(
            NTag,
            { type: verdictType(r.verdict), size: "small" },
            { default: () => r.verdict },
          ),
        ];
        if (r.ai_pending) {
          ...
          children.push(
            h(
              ...
              { default: () => t.submission.aiRejectedTag },
            ),
          );
    ...
    {
      title: t.submission.colTime,
      key: "time_used_ms",
      width: 80,
      render: (r: any) => `${r.time_used_ms} ms`,
    },
    {
      title: t.submission.colMemory,
      key: "memory_used_kb",
      width: 100,
      render: (r: any) => `${r.memory_used_kb} KB`,
    },
...
<template>
  <div>
    <NSpace v-if="showFilters" class="mb-3">
      <NSelect
        v-model:value="verdict"
        :options="verdictOptions"
        class="w-40"
        :placeholder="t.submission.filterVerdict"
      />
      <NInput
        v-if="!hideFiltersUser"
        v-model:value="userFilter"
        :placeholder="t.submission.filterUser"
        class="w-40"
        clearable
        @keyup.enter="
          page = 1;
      ...
      <NInput
        ...
        :placeholder="t.submission.filterProblemId"
        class="w-32"
        clearable
        @keyup.enter="
          page = 1;
