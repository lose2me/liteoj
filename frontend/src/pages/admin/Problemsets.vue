...
import ProblemsetEditorModal from "../../components/admin/ProblemsetEditorModal.vue";
import ProblemsetItemsModal from "../../components/admin/ProblemsetItemsModal.vue";
import ProblemsetMembersModal from "../../components/admin/ProblemsetMembersModal.vue";
...
const createEmptyForm = (): ProblemsetEditorForm => ({
  id: 0,
  title: "",
  password: "",
  allowed_langs: [],
...
const editMode = ref<ProblemsetEditMode>("create");
const form = ref<ProblemsetEditorForm>(createEmptyForm());
...
const load = async () => {
  ...
  // 题目候选区需要拉全量——题目列表接口后端上限 100/页，超过则循环翻页，
  // 否则题目 >100 的库里选择器会"消失"一部分题目（右栏也因此找不到 title）。
  const problems: any[] = [];
  for (let page = 1; page < 50; page++) {
    const { data: pd } = await http.get("/problems", {
...
const openUpdate = (r: Row) => {
  ...
  form.value = {
    ...
    enable_idea: !!r.enable_idea,
    enable_solution: !!r.enable_solution,
    enable_ai: !!r.enable_ai,
    enable_bonus: !!r.enable_bonus,
  };
  showEdit.value = true;
};
...
const submit = async () => {
  ...
  const body = {
    ...
    start_time: form.value.start_ts
      ? new Date(form.value.start_ts).toISOString()
      : null,
    end_time: form.value.end_ts
      ? new Date(form.value.end_ts).toISOString()
      : null,
    visible: form.value.visible !== false,
    enable_idea: !!form.value.enable_idea,
    enable_solution: !!form.value.enable_solution,
    enable_ai: !!form.value.enable_ai,
    enable_bonus: !!form.value.enable_bonus,
  };
...
const saveItems = async () => {
  await http.put(`/admin/problemsets/${currentSetId.value}/problems`, {
...
const pickFiltered = computed(() => {
  ...
  return allProblems.value.filter((p: any) => {
    if (
      kw &&
      !(String(p.id).includes(kw) || (p.title || "").toLowerCase().includes(kw))
    )
      return false;
...
const loadMembers = async () => {
  const { data } = await http.get(
    `/admin/problemsets/${membersSetId.value}/members`,
  );
  members.value = data.items || [];
};
const loadBans = async () => {
  const { data } = await http.get(
    `/admin/problemsets/${membersSetId.value}/bans`,
  );
...
const kickMember = (m: any) => {
  dialog.warning({
    ...
    onPositiveClick: async () => {
      await http.delete(
        `/admin/problemsets/${membersSetId.value}/members/${m.user_id}`,
      );
...
const unban = async (b: any) => {
  await http.delete(
    `/admin/problemsets/${membersSetId.value}/bans/${b.user_id}`,
  );
...
const toggleVisible = async (r: Row) => {
  ...
  msg.success(
    data?.visible ? t.problemsetAdmin.openedOk : t.problemsetAdmin.closedOk,
  );
  await load();
};

const fmt = (s?: string | null) =>
  s ? s.replace("T", " ").slice(0, 16) : t.common.empty;
  ...
  if (r.visible === false) {
    tags.push(
      h(
        ...
        { default: () => t.problemset.visibleClosed },
      ),
    );
  } else {
    tags.push(
      h(
        ...
        { default: () => t.problemset.visibleOpen },
      ),
    );
  }
  if (r.has_password) {
...
const renderProgress = (r: Row) => {
  ...
  return h(
    "div",
    { style: "display:flex; align-items:center; gap:8px; width:160px" },
    [
      h(NProgress, {
        type: "line",
        percentage: pct,
      ...
      h(
        "span",
        { style: "font-size:12px; opacity:0.8; white-space:nowrap" },
        `${ac}/${total}`,
      ),
    ],
  );
...
const columns = [
  ...
  {
    title: t.problemset.colStatus,
    key: "status",
    width: 240,
    render: statusTags,
  },
  {
    title: t.problemset.colProgress,
    key: "progress",
    width: 180,
    render: renderProgress,
  },
  {
    title: t.problemset.colTop,
    key: "top",
    width: 160,
    render: (r: Row) =>
      r.top_ac_name
  ...
  {
    title: t.problemset.colStart,
    key: "start_time",
    width: 140,
    render: (r: Row) => fmt(r.start_time as any),
  },
  {
    title: t.problemset.colEnd,
    key: "end_time",
    width: 140,
  ...
  },
  {
    ...
    render: (r: Row) =>
      h(
        "div",
        { class: "row-actions", onClick: (e: Event) => e.stopPropagation() },
        [
          h(
            NSpace,
            {},
            {
              default: () => [
                h(
                  NButton,
                  { size: "tiny", onClick: () => openUpdate(r) },
                  { default: () => t.problemsetAdmin.opEdit },
                ),
                h(
                  NButton,
                  { size: "tiny", onClick: () => openItems(r) },
                  { default: () => t.problemsetAdmin.opProblems },
                ),
                h(
                  NButton,
                  { size: "tiny", onClick: () => openMembers(r) },
                  { default: () => t.problemsetAdmin.opMembers },
                ),
                h(
                  NButton,
                  {
                  ...
                  },
                  {
                    default: () =>
                      r.visible === false
                        ? t.problemsetAdmin.opOpen
                        : t.problemsetAdmin.opClose,
                  },
                ),
                h(
                  NButton,
                  { size: "tiny", onClick: () => duplicate(r) },
                  { default: () => t.problemsetAdmin.opCopy },
                ),
                h(
                  NButton,
                  { size: "tiny", type: "error", onClick: () => remove(r) },
                  { default: () => t.problemsetAdmin.opDelete },
                ),
              ],
            },
          ),
...
const bindRightListRef = (el: Element | null) => {
  rightListRef.value = el instanceof HTMLElement ? el : null;
};
</script>
...
<template>
  <div>
    ...
    <ProblemsetItemsModal
      v-model:show="showItems"
      v-model:pick-search="pickSearch"
      v-model:pick-tag-names="pickTagNames"
      v-model:pick-page="pickPage"
      :pick-filtered-count="pickFiltered.length"
      :tag-options="tagOptions"
      :pick-paged="pickPaged"
      :page-count="Math.ceil(pickFiltered.length / PICK_PAGE_SIZE)"
      :page-size="PICK_PAGE_SIZE"
      :picked-list="pickedList"
      :bind-right-list-ref="bindRightListRef"
      :is-picked="isPicked"
      :label-of="labelOf"
    ...
    <ProblemsetMembersModal
      v-model:show="showMembers"
      v-model:members-tab="membersTab"
      :members="members"
      :bans="bans"
