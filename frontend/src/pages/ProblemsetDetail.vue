...
import ProblemsetBonusSheet from "../components/ProblemsetBonusSheet.vue";
import SubmissionTable from "../components/SubmissionTable.vue";
import { http } from "../api/http";
import { onEvent } from "../api/events";
import { joinProblemset } from "../api/problemsets";
...
const joinSet = async () => {
  try {
    await joinProblemset(Number(route.params.id), ps.value?.has_password ? pwdInput.value : undefined);
...
const countdown = computed(() => {
  ...
  const start = ps.value.start_time
    ? new Date(ps.value.start_time).getTime()
    : 0;
  if (start && nowTs.value < start)
    return {
      type: "info" as const,
      label: t.problemset.cdPending,
      ms: start - nowTs.value,
    };
  if (nowTs.value < end)
    return {
      type: "success" as const,
      label: t.problemset.cdRunning,
      ms: end - nowTs.value,
    };
...
const columns = [
  {
    ...
    render: (p: any) => {
      ...
      return h(
        NTag,
        { type: statusTagType(s), size: "small" },
...
const rankingEndpoint = computed(
  () => `/problemsets/${route.params.id}/ranking`,
);
const problemsetSubsQuery = computed(() => ({
  problemset_id: Number(route.params.id),
}));
...
const activeTab = computed<string>({
  get: () => {
    ...
    if (typeof v !== "string" || !v) return "problems";
    if (v === "bonus" && !ps.value?.enable_bonus) return "problems";
    return v;
  },
  set: (v) => {
...
<template>
  <div v-if="ps">
    <NCard :title="ps.title">
      <NSpace class="mb-3">
        <NTag v-if="ps.has_password" type="warning">{{
          t.problemset.pwdLock
        }}</NTag>
        <NTag v-if="ps.allowed_langs?.length" type="warning">
          {{ t.problemset.langRestricted }}
        </NTag>
        <NTag v-if="ps.enable_bonus" type="info">{{
          t.problemset.bonusTag
        }}</NTag>
        <NTag v-if="ps.top_ac_name" type="success">
          {{ t.problemset.crown(ps.top_ac_name, ps.top_ac_count) }}
        </NTag>
        <NTag v-if="ps.start_time">{{
          t.problemset.startAt(fmt(ps.start_time))
        }}</NTag>
        <NTag v-if="ps.end_time" type="warning">{{
          t.problemset.endAt(fmt(ps.end_time))
        }}</NTag>
      ...
      <div v-if="!isBanned" class="flex items-center gap-3 mt-2">
        ...
        <span class="text-sm">{{
          t.problemset.acOfTotal(stats.ac, stats.total)
        }}</span>
      </div>

      <template v-if="!isMember">
        <template v-if="isBanned">
          <div class="opacity-80 mb-2 text-red-400">
            {{ t.problemset.bannedLocked }}
          </div>
        </template>
        <template v-else>
          ...
          <NSpace>
            <NInput
              ...
              @keyup.enter="joinSet"
            />
            <NButton type="primary" @click="joinSet">{{
              t.problemset.joinBtn
            }}</NButton>
          </NSpace>
        </template>
      </template>
    ...
    <NTabs v-if="isMember" v-model:value="activeTab" class="mt-4" type="line">
      ...
      <NTabPane name="ranking" :tab="t.problemset.tabRanking">
        <div class="opacity-60 text-xs mb-2">
          {{ t.problemset.penaltyHint }}
        </div>
        <Ranking :endpoint="rankingEndpoint" :hide-scope="true" />
      </NTabPane>
      <NTabPane name="submissions" :tab="t.problemset.tabSubs">
        <SubmissionTable
          :query="problemsetSubsQuery"
          :detail-from-ps="true"
          :page-size="16"
        />
      </NTabPane>
      <NTabPane
        v-if="ps.enable_bonus"
        name="bonus"
        :tab="t.problemset.tabBonus"
      >
        <ProblemsetBonusSheet
          :problemset-id="Number(route.params.id)"
