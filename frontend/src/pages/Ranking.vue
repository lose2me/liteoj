...
const load = async () => {
  ...
  bonusEnabled.value = !!data.bonus_enabled;
};
...
const globalColumns = computed(() => {
  ...
  if (user.isAdmin) {
    cols.push({
      title: t.ranking.account,
      key: "username",
      width: 140,
      render: (r: any) =>
        h("span", { style: "white-space:nowrap" }, r.username),
    });
  }
  cols.push(
    {
      title: t.ranking.name,
      key: "name",
      width: 140,
...
const buildProblemsetColumns = () => {
  ...
  if (bonusEnabled.value) {
    head.push({
      ...
      key: "bonus_score",
      width: 90,
      render: (r: any) => r.bonus_score || 0,
    });
  }
  head.push({
    title: t.ranking.penalty,
    key: "penalty_min",
    width: 90,
    render: (r: any) => `${r.penalty_min || 0} min`,
  });
