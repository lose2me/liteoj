...
func ensurePerformanceIndexes(db *gorm.DB) error {
	for _, sql := range []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_problem_sets_bonus ON problem_sets (enable_bonus)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_ps_bonus_ps_date ON problem_set_daily_bonus (problem_set_id, score_date)`,
		`CREATE INDEX IF NOT EXISTS idx_ps_bonus_user ON problem_set_daily_bonus (user_id)`,
